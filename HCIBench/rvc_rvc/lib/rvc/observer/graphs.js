...
function computeWdtGraphSpecsInt(rawdata, prefix, thumb) {
    ...
    var graphSpecs = []
    var yAxisLabel = rawdata['wdt']
    var yAxisLabelReplace = {
        "VSAN_": "",
        "tq-vit": "tq:vit",
    }
    for (var key in yAxisLabelReplace) {
        yAxisLabel = yAxisLabel.replace(key, yAxisLabelReplace[key])
    }
    if (!thumb) {
        graphSpecs = [
            [yAxisLabel, "wdt",
...
// Generate stats full graphs for VSAN iSCSI Target
function computeVitGraphSpecs(rawdata, prefix) {
  return computeVitGraphSpecsInt(rawdata, prefix, false)
}

// Generate stats thumbnail graphs for VSAN iSCSI Target
function computeVitGraphSpecsThumb(rawdata, prefix) {
  return computeVitGraphSpecsInt(rawdata, prefix, true)
}

function computeVitGraphSpecsInt(rawdata, prefix, thumb) {
  var data = []
  var labels = []
  var graphSpecs = []

  // stats key list in each VSI node
  statsKeyList = ['vitdRestartCount']
  // Y-axis label for each graph
  yAxisLabel = ['vitd restart count']
  statsData = rawdata['stats']
  for (var i in statsKeyList) {
    d = convertInputs(statsData['vit.vitdRestartCount'],
                      statsKeyList[i], 'total', function(v){return v;})
    data.push(d)
    labels.push(statsKeyList[i])
    graphSpecs.push([
      yAxisLabel[i], statsKeyList[i],
      prefix + "-" + statsKeyList[i],
      data,
      labels,
      null,
      -1
...
// Generate target stats full graphs for VSAN iSCSI Target
function computeVitTargetGraphSpecs(rawdata, prefix) {
  return computeVitTargetGraphSpecsInt(rawdata, prefix, false)
}

// Generate target stats thumbnail graphs for VSAN iSCSI Target
function computeVitTargetGraphSpecsThumb(rawdata, prefix) {
  return computeVitTargetGraphSpecsInt(rawdata, prefix, true)
}

function computeVitTargetGraphSpecsInt(rawdata, prefix, thumb) {
  var data = []
  var labels = []
  var graphSpecs = []
  ...
  var targetStats = [
    ['numLoginAttempt', 'numLoginFailure'],
  ]
  graphLabels = [
    ['Login Attempts', 'Login Failures'],
  ]
  yAxisLabel = [
    '',
  ]
  ...
  for (var i in targetStats) {
    data = []
    labels = []
    for (var j in targetStats[i]) {
      d = convertInputs(statsData,
                        targetStats[i][j], 'total', function(v){return v;})
      data.push(d)
      labels.push(graphLabels[i][j])
    }
    graphSpecs.push([
      thumb ? yAxisLabelThumb[i] : yAxisLabel[i],
      graphTypes[i],
      prefix + "-" + graphTypes[i],
      data,
      labels,
...
// Generate LUN stats full graphs for VSAN iSCSI Target
function computeVitLunGraphSpecs(rawdata, prefix) {
  return computeVitLunGraphSpecsInt(rawdata, prefix, false)
}

// Generate LUN stats thumbnail graphs for VSAN iSCSI Target
function computeVitLunGraphSpecsThumb(rawdata, prefix) {
  return computeVitLunGraphSpecsInt(rawdata, prefix, true)
}

function computeVitLunGraphSpecsInt(rawdata, prefix, thumb) {
  var data = []
  var labels = []
  var graphSpecs = []
  ...
  // each line as a separate graph
  var prStats = [
    ['isReserved'],
    ['numActiveRegistrants'],
  ]
  // Y-axis label for each full graph
  yAxisLabel = [
    'isReserved',
    'Number of active registrants',
  ]
  // Y-axis label for each thumbnail
  yAxisLabelThumb = [
    'isReserved',
    'numRegistrants',
  ]
  // graphTypes to match in loadGraphs and replaceElementWithMiniGraphs
  graphTypes  = [
    'isReserved',
    'numActiveRegistrants',
  ]
  for (var i in prStats) {
    data = []
    labels = []
    for (var j in prStats[i]) {
      d = convertInputs(statsData['prstats'],
                        prStats[i][j], 'total', function(v){return v;})
      data.push(d)
      labels.push(prStats[i][j])
    }
    graphSpecs.push([
      thumb ? yAxisLabelThumb[i] : yAxisLabel[i],
      graphTypes[i],
...
// filter out iSCSI targets by target alias against the input query string
app.filter('filterTarget', function() {
  return function(targets, query) {
    var matchedTargets = {};
    angular.forEach(targets, function(targetInfo, targetAlias) {
      // if the query is a substring of targetAlias (valid index in targetAlias)
      if (targetAlias.indexOf(query) > -1) {
          matchedTargets[targetAlias] = targetInfo;
      }
    });
    var out = {
      'numTargets': Object.keys(targets).length,
      'numMatchedTargets': Object.keys(matchedTargets).length,
...
app.controller("VmListCtrl", function($scope, $http) {
  ...
  $scope.showVitTab = false;
  $scope.showLuns = false;
  $scope.hosts = hosts;
  ...
  $scope.$watch("showVitTab", function(newVal) {
    if (newVal != true) {
      return;
    }
    $http.get('jsonstats/vit/vit-list.json').success(function(data) {
      $scope.vitlist = data;
      for (var target in $scope.vitlist) {
        // add 'showluns' property to each target
        // do not expand LUNs info for a target by default
        $scope.vitlist[target]['showLuns'] = false
      }
    });
  })
  ...
  $scope.toggleAllTargets = function() {
    $scope.showLuns = !$scope.showLuns
    for (var target in $scope.vitlist) {
      $scope.vitlist[target].showLuns = $scope.showLuns
    }
  }
  $scope.toggleTargetShowLuns = function(target) {
      target.showLuns = !target.showLuns
  }
...
app.directive('vitHostFullGraph', function() {
  return {
    // Only used as attribute
    restrict: "A",
    replace: true,
    scope: {
      hostname: '@',
    },
    template: "<a href='graphs.html?group=vit&host={{hostname}}'> Full graphs </a>",
  };
});

app.directive('vitHostThumbGraph', function($http) {
  return {
    ...
    link: function(scope, element, attrs) {
      var graphTypes = ["vitdRestartCount"];
      var prefix = "vit-vsi-" + scope.hostname;
      replaceElementWithMiniGraphs($http, element, prefix, "jsonstats/vit/", graphTypes, computeVitGraphSpecsThumb);
    },
  };
});

app.directive('vitHostGraph', function($http, $filter) {
  return {
    // Only used as attribute
    restrict : "A",
    replace: true,
    ...
    template: ["<table class='graphgrid' style='text-algign: left;'>",
               "  <tbody>",
               "  <tr>",
               "    <td style='padding-right: 15px; width: 190px;' valign='top'>",
               "      <span>{{hostname}}</span><br>",
               "      <a vit-host-full-graph hostname={{hostname}}/>",
               "    </td>",
               "    <td style='padding-right: 15px;' valign='top'>",
               "     <vit-host-thumb-graph hostname={{hostname}}></vit-host-thumb-graph>",
               "    </td>",
               "  </tr>",
               "  </tbody>",
               "</table>", ].join(""),
...
app.directive('vitTargetFullGraph', function() {
  return {
    restrict: "A",
    replace: true,
    scope: {
    ...
    template: "<a href='graphs.html?group=vittarget&alias={{alias}}'> Full graphs </a>",
  };
});

app.directive('vitTargetThumbGraph', function($http) {
  return {
    restrict: "E",
    scope: {
      alias: '@',
    ...
    link: function(scope, element, attrs) {
      ...
      var prefix = "vit-vsi-target-" + scope.alias;
      replaceElementWithMiniGraphs($http, element, prefix, "jsonstats/vit/", graphTypes, computeVitTargetGraphSpecsThumb);
    },
  };
});

app.directive('vitTargetStats', function($http, $filter, $compile) {
  return {
    restrict : "AEC",
    replace: true,
    scope : {
    ...
    link : function(scope, _element, attrs) {
      ...
      scope.$watch("targetInfo", function(newVal, oldVal) {
        updateDom()
      })

      function updateDom() {
        if (scope.targetInfo == null) {
          return;
        }
        ...
        var iqn = scope.targetInfo['target-iqn']
        var alias = scope.targetInfo['target-alias']
        var html = [
          "Target IQN: " + iqn,
           "",
           "Target stats: <div vit-target-full-graph alias='" + alias + "'></div>",
           "<vit-target-thumb-graph alias='" + alias + "'></vit-target-thumb-graph>",
        ]
...
app.directive('vitLunFullGraph', function() {
  return {
    // Only used as attribute
    restrict: "A",
    replace: true,
    ...
    template: "<a href='graphs.html?group=vitlun&uuid={{uuid}}'> Full graphs </a>",
  };
});

app.directive('vitLunThumbGraph', function($http) {
  return {
    // Only used as element
    restrict: "E",
    scope: {
    ...
    link: function(scope, element, attrs) {
      var graphTypes = ["isReserved", "numActiveRegistrants"];
      var prefix = "vit-vsi-lun-" + scope.uuid;
      replaceElementWithMiniGraphs($http, element, prefix, "jsonstats/vit/", graphTypes, computeVitLunGraphSpecsThumb);
    },
  };
});

// Directive to display LUN stats for VSAN iSCSI Target
// This directive is adapted from 'virtualDisk' directive
app.directive('vitLunStats', function($http, $filter, $compile, vsansparseSvc) {
  return {
    // matches either attribute or class name
    restrict : "AC",
    scope : {
      lunInfo : '=lunInfo',
    },
    link : function(scope, _element, attrs) {
      ...
      scope.$watch("lunInfo", function(newVal, oldVal) {
        updateDom()
      })
      ...
      scope.toggleShowBacking = function (lunUuid) {
        scope.showdetails[lunUuid] = !(scope.showdetails[lunUuid] == true)
        updateDom()
      }
      ...
      function updateDom() {
        ...
        var lunUuid = scope.lunInfo['lun-uuid']
        var lunId = scope.lunInfo['lun-id']
        var lunAlias = scope.lunInfo['lun-alias']
        var lunAliasText = ""
        if (lunAlias) {
          lunAliasText = " [" + lunAlias + "]"
        }
        ...
        element.append("div")
          ...
          .html("<i ng-hide=\"showbackings == true\" class=\"icon-caret-right\"></i><i ng-show=\"showbackings == true\" class=\"icon-caret-down\"></i> LUN " + lunId + lunAliasText + "<br>")
        if (scope.showbackings) {
          var html = [
            "LUN Size: " + scope.lunInfo['lun-size'] + " MiB",
            "",
            "LUN stats: <div vit-lun-full-graph uuid='"+lunUuid+"'></div>",
            "<vit-lun-thumb-graph uuid='"+lunUuid+"'></vit-lun-thumb-graph>",
          ]
          element.append("div")
            .attr("class", "objectDetails")
          ...
          drawBacking(ul, lunUuid, lunId, lunAlias)
        }
        $compile(_element.contents())(scope)
      }

      function drawBacking(element, lunUuid, lunId, lunAlias) {
        var vmdkName = lunUuid + ".vmdk"

        element.append("li")
          .attr("class", "clickable")
          .attr("ng-click", "toggleShowBacking('"+lunUuid+"')")
          .attr("style", "margin-left: 20px")
          .html("<i ng-hide=\"showdetails[lunUuid] == true\" class=\"icon-caret-right\"></i><i ng-show=\"showdetails[lunUuid] == true\" class=\"icon-caret-down\"></i> Backing: " + vmdkName + "<br>")
        if (scope.showdetails[lunUuid] == true) {
          var url = "graphs.html?json=jsonstats/dom/domobj-" + lunUuid + ".json"
          var html = [
            "DOM Object UUID: " + lunUuid + " <a href='"+url+"'>Full graphs</a>",
            "",
            "DOM owner:",
            "<div dom-graph dom-graph-uuid='"+lunUuid+"'></div>",
            "RAID tree:",
            "<div raid-tree raid-tree-obj-uuid='"+lunUuid+"'></div>",
          ]
          if (vsansparseSvc.isVsanSparse(lunUuid)) {
            html.push("<vsansparse-disk backing=\"'"+ lunUuid + "'\">");
          }
          element.append("div")
            .attr("class", "objectDetails")
            .attr("style", "margin-left: 20px; margin-bottom: 20px")
