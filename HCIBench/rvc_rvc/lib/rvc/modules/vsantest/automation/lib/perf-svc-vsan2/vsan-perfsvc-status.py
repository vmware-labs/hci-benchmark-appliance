...
if onEsx:
   ...
   from VsanHealthUtil import ConnectToLocalHostd, IsVsanMaxEnabledInHost
   from VsanHostHelper import isWitnessHost, isVsanPerfWitnessMetricsEnabled, isVsanPerfHighResolutionEnabled, \
                              HOST_ENTITY_TYPES, OBJECT_ENTITY_TYPES
else:
...
perf_stats_ioinsight  [days|hours]           Print vSAN IOInsight stats, argument is number of days, by default print 2 days.
perf_stats_with_dump_ioinsight  [days|hours] Print vSAN IOInsight stats in SOAP format, argument is number of days, by default print 2 hours.
...
if IsVsanMaxEnabledInHost():
   supportBundlePriorityMetrics["P0"].update({
      ...
      "vsan-esa-dom-capacity-fullness": allFields,
   })
   supportBundlePriorityMetrics["P0"].update({
