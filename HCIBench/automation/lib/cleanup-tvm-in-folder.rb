...
begin
	puts `govc find -type m -i -dc "#{Shellwords.escape($dc_name)}" . -parent "#{tvm_folder_moid}" | { read -N1 line && { echo -n "$line"; cat; }| xargs govc vm.destroy -dc "#{Shellwords.escape($dc_name)}"; }`, @tvm_cleanup_log
