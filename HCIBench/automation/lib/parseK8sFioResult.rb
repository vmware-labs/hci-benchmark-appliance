...
if ARGV.empty?
  ...
  ARGV.each do |dir|
    if File.directory?(dir)
      ...
      vm_finish_early = 0
      files_finished_early = []
      file_arr = Dir.glob("#{dir}/*-k8s-*.json").sort
      ...
      puts "StorageClass  = #{sc_label}", resfile
      puts "All the Pods finished testing early, no results summarized", resfile if max_num_jobs == 0
      for i in 0..max_num_jobs-1
        ...
        puts "Pods\t\t= #{num_vm-vm_finish_early}", resfile
