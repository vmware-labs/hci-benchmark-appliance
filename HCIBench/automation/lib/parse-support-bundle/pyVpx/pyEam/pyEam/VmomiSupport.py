...
from . import Iso8601
from pyVmomi import _allowGetSet
from pyVmomi import _allowCapitalizedNames
...
try:
    ...
    # we need to tolerate the import error here. (See PR 2704619)
    def intern(s):
        return s
...
def FormatObject(val, info=Object(name="", type=object, flags=0), indent=0):
    ...
    if val is None:
        ...
        if val:
            ...
            result = "(%s) [\n%s\n%s]" % (itemType.__name__, ',\n'.join(
                [FormatObject(obj, item, indent + 1)
                 for obj in val]), indent * "   ")
        else:
...
def _areBasicTypes(info, valType):
    ...
    return (info is type and valType is type(Exception)
            ...
            or issubclass(info, long) and
            (issubclass(valType, int) or issubclass(valType, long))
            or issubclass(info, float) and issubclass(valType, float)
            or issubclass(info, string_types)
            and issubclass(valType, string_types) or
            issubclass(info, binary_type) and issubclass(valType, binary_type))
...
# generated bindings so removing it will break the world. See PR 2603708
