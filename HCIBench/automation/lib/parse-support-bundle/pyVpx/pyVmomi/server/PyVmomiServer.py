...
from .Security import SetAuthChecker
...
def _SetupLogging(options):
    """Setup debug logging"""
    _LoggingFactory.ParseOptions(options)
...
def _ParseArguments(argv):
    ...
    _LoggingFactory.AddOptions(cmdParser)
...
def Initialize(authChecker, options=None, remainingOptions=None, validatorList=None):
    """
    Initialize the server, including global options and validators
    :param authChecker: A service-specific authentication checker
    :type  authChecker: Security.AuthChecker
    :param options: CMD options
    :type  options: optparse.Values
    :param remainingOptions: The leftover positional arguments after all options have been processed
    :param validatorList: A list of service-specific validators
    :type  validatorList: SoapHandler.Validator
    """
    global _gInitialized
    if _gInitialized:
        return
    ...
    if validatorList is not None:
        if hasattr(validatorList, '__iter__'):
            global gValidatorList
            gValidatorList = validatorList
        else:
            raise TypeError("validatorList is not iterable")
...
def _ImportTypesAndManagedObjects():
...
if __name__ == "__main__":
    gCmdOptions, gCmdRemainingOptions = _ParseArguments(sys.argv[1:])
    _SetupLogging(gCmdOptions)
    Initialize(None, options=gCmdOptions, remainingOptions=gCmdRemainingOptions)
...
class _GeneralHttpHandler(BaseHTTPRequestHandler):
    """Http handler"""
    ...
    def do_POST(self):
            ...
            if contentLength:
                try:
                    ...
                    if contentLength > maxContentLen:
                        ...
                        # length
                        logging.warning("Request content length %d > %d" % \
        ...
        try:
            ...
            if isinstance(response, str):
                ...
                isStringResponse = isinstance(response, bytes)
            ...
            _LogException("Error: Send response exception: ", err)
            self.close_connection = 1
    ...
    def process_request_worker(self):
        ...
        while self.thread_looping:
            try:
                ...
                _LogException("Thread caught exception: ", err)
                time.sleep(1)
...
class SoapHttpServer(HttpServerThreadPoolMixin, HTTPServer):
    ...
    def shutdown(self):
        ...
        logging.debug("closing down listener socket")
...
class _ScgiServer(object):
    ...
    def _HandleOneSCGIRequest(self):
        ...
        try:
            ...
            handler = _CgiBaseHandler(req.headers, self.rfile, responsefile,
                                     self.stubs)
            ...
            return _CgiBaseHandler._CgiResponse('Missing header key: %s' %
                                               (e)).encode(UTF8)
        except Scgi.ScgiError as e:
            return _CgiBaseHandler._CgiResponse('SCGI parsing error: %s' %
                                               (e)).encode(UTF8)
        except Exception as e:
            import traceback
            return _CgiBaseHandler._CgiResponse(
                'General exception: %s\n%s' %
...
class _LoggingFactory:
    ...
    def ParseOptions(options):
        ...
        maxBytes = _LoggingFactory.logSizeMB * 1024 * 1024
        backupCount = _LoggingFactory.numFiles - 1
        ...
        if options.syslogident:
            try:
                ...
                syslogFormat = options.syslogident+': '+ _LoggingFactory.logFormat
        ...
        if handler:
            ...
            formatter = logging.Formatter(_LoggingFactory.logFormat)
...
class _ServerMain:
    ...
    def Stop(self):
        ...
        if getattr(self.options, 'scgi', False) and self.scgid is not None:
            self.scgid.shutdown()
        elif self.httpd is not None:
            self.httpd.shutdown()
        else:
    ...
    def _RunServer(self):
        ...
        if getattr(self.options, 'interactive', False):
            ...
            handler = _CgiBaseHandler(os.environ, self.stdin, self.stdout,
                                     stubs)
            try:
            ...
            self.scgid = _ScgiServer(stubs=stubs)
            self.scgid.serve_forever()
            sys.exit(0)
        else:
            # Handle SOAP request from HTTP
            if getattr(self.options, 'unix', None):
                addr = self.options.unix
                soapHttpdConstructor = _SoapHttpServerOverUnixSocket
            ...
            soapHttpd = soapHttpdConstructor(addr, _GeneralHttpHandler)
            self.httpd = soapHttpd
            if soapHttpdConstructor == _SoapHttpServerOverUnixSocket:
                # Make sure we cleanup the unix socket file
                import atexit
                atexit.register(_CleanupUnixSocket, addr)
...
def Run():
    ...
    try:
        if not _gInitialized:
            raise RuntimeError("The server is not initialized")

        SoapHandler.Setup()
        if gValidatorList is not None:
            for validator in gValidatorList:
                SoapHandler.RegisterValidator(validator)
        global gServerMain
        gServerMain = _ServerMain()
        gServerMain.Start(gCmdOptions, gCmdRemainingOptions)
    except Exception as err:
        message = str(err)
        if message:
            print(message)
