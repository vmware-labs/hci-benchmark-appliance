...
from pyVmomi.server.MoManager import GetMoManager
from pyVmomi.server.DynTypeMgr import GetDynTypeMgr
import logging
...
class DynamicTypeManager(vmodl.reflect.DynamicTypeManager):
   ...
   for name, translated in _translatedTypeName.copy().items():
   ...
   def QueryMoInstances(self, filterSpec=None):
      ...
      for (moId, serverGuid), instance in objects.items():
   ...
   def _CreateManagedTypeInfo(self, aType):
      ...
      for info in aType._propInfo.values():
      ...
      for info in aType._methodInfo.values():
