...
puts $datastore_names[0],@log_file
total_capacity, free_space = _get_datastore_capacity_and_free_space($datastore_names[0])
total_capacity_size = total_capacity.to_i
total_usable_capacity = free_space.to_i 

if test_vsan
  vsan_datastore_name = $datastore_names[0]
  local_type = vsan_datastores[vsan_datastore_name]["local"] ? "Local" : "Remote"
  ...
  puts "
    Datastore Name: #{$datastore_names[0]}
    Total Capacity: #{total_capacity_size}
    Total Usable Capacity: #{total_usable_capacity}", @log_file
    disk_init = "RANDOM"
    host_num = (_get_hosts_list & _get_hosts_list_by_ds_name($datastore_names[0])).count 
end
    

if test_vsan and $vsan_version == 1 and vsan_type == "All-Flash"
  total_cache_size = [num_of_dg * 600,total_cache_size].min
  ratio = 0.75
elsif $vsan_version == 2 or not test_vsan
...
while (total_usable_capacity * 0.8) < total_cache_size
...
$latency_target_hash = {
  ...
  "Medium" => {"random" => 5000, "sequential" => 10000},
  "High" => {"random" => 10000, "sequential" => 20000}
}

@data_disk_num = ($latency_target == "Max") ? 8 : 4

if test_vsan and $vsan_version == 1
  @vm_num = num_of_dg * 2
...
puts "
Number of VMs: #{@vm_num}
Number of Disks per VM: #{@data_disk_num}
Disk Size: #{@disk_size}
Thread Count per Disk: #{thread_num}", @log_file 

workload_profiles = []
seq = test_vsan ? $vsan_version : 2
for workload in $workloads
  workload_profiles.append(workload_profiles_hash[workload][seq])
end
...
puts workload_profiles,@log_file
workloadParam = ""
